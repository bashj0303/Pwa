...
import { Plus, Trash2, Flame, RotateCcw, Settings, Target, Zap, Bot, Loader2, Camera, X, ArrowLeft, Calculator, User, Barcode, ScanLine } from 'lucide-react';
...
// ─── NOUVEAU SCANNER PRO (Rapide & Précis) ────────────────────
const BarcodeScanner = ({ onDetected, onClose, isFr }) => {
  ...
  useEffect(() => {
    ...
    const startScanner = async () => {
      try {
        // Injection dynamique de la librairie pro (html5-qrcode)
        ...
        // Configuration optimisée pour les codes-barres 1D (boîte rectangulaire)
        await html5QrCode.start(
          ...
          { 
            ...
            aspectRatio: 1.0,
            formatsToSupport: [ window.Html5QrcodeSupportedFormats.EAN_13, window.Html5QrcodeSupportedFormats.UPC_A ]
          },
          (decodedText) => {
            if (html5QrCode.isScanning) {
              html5QrCode.stop().then(() => onDetected(decodedText)).catch(console.error);
            }
          },
          (errorMessage) => {
            // On ignore les erreurs de scan frame par frame, c'est normal
          }
        );
      } catch (err) {
        console.error(err);
        setError(isFr ? "Impossible d'ouvrir la caméra. Autorise l'accès." : "Cannot access camera. Please allow permissions.");
      }
    };
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-50 flex flex-col animate-in fade-in duration-200 backdrop-blur-sm">
      <div className="flex-1 flex flex-col justify-center items-center px-4">
        <h3 className="text-[#ccff00] font-black text-xl mb-6 tracking-widest uppercase">
          {isFr ? 'Scan ton aliment' : 'Scan your food'}
        </h3>
        
        <div className="w-full max-w-sm relative">
          {/* Le container du scanner */}
          <div id="reader" className="w-full rounded-[2rem] overflow-hidden border-4 border-[#ccff00]/50 shadow-[0_0_30px_rgba(204,255,0,0.2)] bg-black min-h-[300px]"></div>
        </div>

        {error && (
          <div className="mt-6 bg-red-900/50 border border-red-500/50 p-4 rounded-xl text-center">
      ...
      <div className="p-6 pb-12">
        <button 
          onClick={onClose} 
          className="w-full bg-slate-800 border border-slate-700 text-white font-black py-4 rounded-2xl hover:bg-slate-700 transition-colors uppercase tracking-widest text-sm"
        >
          {isFr ? 'Annuler' : 'Cancel'}
        </button>
      </div>
...
const Nutrition = ({ t }) => {
  const isFr = useMemo(() => {
    ...
    if (str.includes('entraînement') || str.includes('diète') || str.includes('accueil')) return true;
    return false;
  }, [t]);
  ...
  const vocab = useMemo(() => ({
    ...
    setGoalsTitle: isFr ? 'Objectifs' : 'Goals',
    namePlaceholder: isFr ? 'Ex: Riz' : 'Ex: Rice',
    qtyPlaceholder: isFr ? 'Qté' : 'Qty',
    emptyState: isFr ? 'Aucun repas ce jour-là. Commence ton grind.' : 'No meals for this day. Start your grind.',
    ...
    aiCoachPlaceholder: isFr ? 'ex: 2 cuillères de beurre de pinotte...' : 'e.g. 2 spoons of peanut butter...',
    aiBtnScan: isFr ? 'Analyser' : 'Analyze',
    aiLoading: isFr ? 'Analyse...' : 'Analyzing...',
    aiClose: isFr ? 'Fermer IA' : 'Close AI',
    aiOpen: isFr ? 'Gemini IA ✨' : 'Gemini AI ✨',
    saveBtn: isFr ? 'Enregistrer' : 'Save Goals',
    calories: isFr ? 'Calories' : 'Calories',
    protein: isFr ? 'Protéines' : 'Protein',
    carbs: isFr ? 'Glucides' : 'Carbs',
    fat: isFr ? 'Lipides' : 'Fat',
    calcTitle: isFr ? 'Macros' : 'Macros',
    ...
    job1: isFr ? 'Sédentaire (Bureau)' : 'Sedentary (Desk)',
    job2: isFr ? 'Léger (Debout/Marche)' : 'Light (Standing/Walking)',
    job3: isFr ? 'Très Actif (Physique)' : 'Very Active (Physical)',
    ...
    calcBtnText: isFr ? 'Générer' : 'Generate',
    scanBtn: isFr ? 'Scan Code' : 'Scan Code',
    scanLoading: isFr ? 'Recherche...' : 'Searching...',
    scanSuccess: isFr ? 'Trouvé ! ✅' : 'Found ! ✅',
    scanNotFound: isFr ? '❌ Introuvable.' : '❌ Not found.',
    scanError: isFr ? 'Erreur réseau.' : 'Network error.',
  }), [isFr]);
  ...
  // ── REQUÊTE OPEN FOOD FACTS ROBUSTE ─────────────────────────────
  const handleBarcodeDetected = async (barcode) => {
    ...
    try {
      const res = await fetch(`https://world.openfoodfacts.org/api/v2/product/${cleanBarcode}?fields=product_name,brands,nutriments,serving_size,serving_quantity`);
      ...
      if (data.status === 1 && data.product) {
        ...
        // Extraction précise : on favorise les kcal, sinon on convertit les kJ
        let k100 = n['energy-kcal_100g'] || n['energy-kcal_value'];
        if (!k100 && n['energy_100g']) k100 = n['energy_100g'] / 4.184; 
        
        const p100 = n['proteins_100g'] || 0;
        const c100 = n['carbohydrates_100g'] || 0;
        const f100 = n['fat_100g'] || 0;

        const productName = p.product_name || p.brands || (isFr ? 'Produit scanné' : 'Scanned product');
        
        // Extraction de la portion suggérée
        let servingGrams = null;
        if (p.serving_quantity) servingGrams = String(Math.round(Number(p.serving_quantity)));

        setScannedProduct({
          name: productName,
          brand: p.brands || '',
          k100: Math.round(k100 || 0),
          p100: Number(p100.toFixed(1)),
          c100: Number(c100.toFixed(1)),
          f100: Number(f100.toFixed(1)),
  ...
  return (
    <div className="space-y-4 pb-64 animate-in fade-in duration-300">
      ...
      {scanStatus && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full font-black text-sm shadow-2xl flex items-center gap-3 animate-in slide-in-from-top duration-300 border ${
          scanStatus === 'loading' ? 'bg-slate-800 border-slate-700 text-white' :
          scanStatus === 'success' ? 'bg-[#ccff00] border-[#a3cc00] text-black' :
          'bg-red-500 border-red-600 text-white'
        }`}>
          {scanStatus === 'loading' && <Loader2 size={18} className="animate-spin" />}
      ...
      {scannedProduct && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-end justify-center animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-[#0f172a] border-t border-slate-800 rounded-t-[2.5rem] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 pb-10">
            
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1 pr-4">
                <span className="inline-block text-[10px] font-black text-[#ccff00] bg-[#ccff00]/10 border border-[#ccff00]/20 px-2 py-0.5 rounded-full uppercase tracking-wider mb-2">
                  CODE-BARRES TROUVÉ
                </span>
                <h3 className="text-white font-black text-xl leading-tight">{scannedProduct.name}</h3>
                {scannedProduct.brand && <p className="text-slate-400 text-xs mt-1 font-bold">{scannedProduct.brand}</p>}
              </div>
              <button onClick={() => setScannedProduct(null)} className="p-2 bg-slate-800 rounded-full text-slate-400 hover:text-white transition-colors">
                <X size={20} />
              </button>
            </div>

            <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 mb-6">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-3 text-center">{isFr ? 'Macros pour 100g' : 'Macros per 100g'}</p>
              <div className="flex justify-between px-2">
                {[
                  { l: 'Kcal', v: scannedProduct.k100, c: 'text-orange-400' },
                  { l: 'Prot', v: `${scannedProduct.p100}g`, c: 'text-blue-400' },
                  { l: 'Gluc', v: `${scannedProduct.c100}g`, c: 'text-amber-400' },
                  { l: 'Lip', v: `${scannedProduct.f100}g`, c: 'text-purple-400' },
                ].map((s, i) => (
                  <div key={i} className="flex flex-col items-center">
                    <span className={`text-lg font-black ${s.c}`}>{s.v}</span>
                    <span className="text-[9px] font-bold text-slate-400 uppercase mt-1">{s.l}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block text-center">
                {isFr ? 'Quantité mangée (en grammes)' : 'Quantity eaten (grams)'}
              </label>

              <div className="flex gap-2 mb-4">
                {(scannedProduct.servingSize ? [scannedProduct.servingSize, '50', '100', '200'] : ['30', '50', '100', '200']).map((q, i) => (
                  <button
                    key={i}
                    onClick={() => setScanQty(String(q))}
                    className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${
                      String(scanQty) === String(q)
                        ? 'bg-slate-700 text-white border-2 border-[#ccff00]'
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-900 border-2 border-slate-700 focus:border-[#ccff00] rounded-2xl px-4 py-4 text-white font-black text-center text-2xl outline-none transition-colors"
                />
                <span className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-lg">g</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full bg-[#ccff00] disabled:bg-slate-800 disabled:text-slate-600 text-black font-black py-4 rounded-xl flex justify-center items-center gap-2 hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(204,255,0,0.2)] uppercase tracking-widest"
            >
              <Plus size={20} /> {isFr ? `Ajouter ${scanQty || 0}g` : `Add ${scanQty || 0}g`}
            </button>
          </div>
        </div>
      )}
      ...
      {/* DASHBOARD HABITUEL */}
      <div className="bg-[#121212] border border-slate-800 p-4 sm:p-5 rounded-[2rem] shadow-xl relative">
        ...
        <div className="flex justify-between items-center bg-black/40 p-1 rounded-xl border border-slate-800 mb-5 overflow-x-auto hide-scrollbar">
          {vocab.days.map((d, i) => (
      ...
      <div className="space-y-3 px-1">
        ...
        {(weeklyFoods[activeDay] || []).map((item) => (
          <div key={item.id} className="bg-[#121212] border border-slate-800 p-3 rounded-2xl flex items-center shadow-md">
            <div className="pr-3 border-r border-slate-800 mr-3 text-center w-12 shrink-0"><span className="block text-[10px] font-black text-[#ccff00]">{item.time}</span></div>
      ...
      {/* BARRE D'AJOUT FIXE */}
      <div className="fixed bottom-[85px] left-1/2 -translate-x-1/2 w-full max-w-md bg-[#0a0a0a]/95 backdrop-blur-xl border-y border-slate-800 p-3 sm:p-4 z-30 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        <div className="w-full space-y-2 sm:space-y-3">
          ...
          <div className="flex justify-center gap-2 -mt-8 relative z-40">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-full font-black text-[11px] tracking-widest shadow-[0_0_15px_rgba(204,255,0,0.2)] bg-[#ccff00] text-black hover:scale-105 transition-transform uppercase"
            >
              <ScanLine size={16} /> {vocab.scanBtn}
            </button>
            <button
              onClick={() => setShowAIPanel(!showAIPanel)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-black text-[11px] tracking-widest shadow-lg transition-transform hover:scale-105 uppercase ${showAIPanel ? 'bg-slate-800 text-white border border-slate-700' : 'bg-slate-800 border border-slate-700 text-white'}`}
            >
              <Bot size={16} className={showAIPanel ? "text-slate-400" : "text-blue-400"} /> {showAIPanel ? vocab.aiClose : vocab.aiOpen}
            </button>
          </div>

          {showAIPanel && (
            <div className="bg-[#121212] p-3 rounded-xl border border-slate-800 animate-in fade-in zoom-in duration-200">
              <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-2 block">{vocab.aiCoachTitle}</label>
              {imagePreview && (
                <div className="relative mb-3 inline-block">
                  <img src={imagePreview} alt="Meal" className="h-24 w-24 object-cover rounded-lg border-2 border-blue-500" />
              ...
              <div className="flex gap-2 mb-2">
                <textarea value={aiInput} onChange={(e) => setAiInput(e.target.value)} placeholder={vocab.aiCoachPlaceholder} className="flex-1 bg-[#0a0a0a] border border-slate-800 rounded-lg p-2 sm:p-3 text-[16px] text-white focus:border-blue-500 outline-none resize-none h-12 sm:h-14" />
                <div className="flex items-center justify-center shrink-0">
                  ...
                  <label htmlFor="camera-upload-nutri" className="bg-slate-800 hover:bg-slate-700 cursor-pointer h-12 w-12 sm:h-14 sm:w-14 rounded-lg flex items-center justify-center text-blue-400 border border-slate-700 transition-colors"><Camera size={20} className="sm:w-6 sm:h-6" /></label>
                </div>
              </div>
              <button onClick={handleAIAnalyze} disabled={isAiLoading || (!aiInput.trim() && !imageBase64)} className="w-full bg-blue-600 disabled:bg-slate-800 text-white font-black py-2.5 rounded-lg flex items-center justify-center gap-2 uppercase tracking-widest text-[11px]">
          ...
          {!showAIPanel && (
            <>
              <div className="flex gap-1.5 sm:gap-2">
                <input type="time" value={timeStr} onChange={e => setTimeStr(e.target.value)} className="w-[65px] sm:w-20 shrink-0 bg-[#121212] border border-slate-800 rounded-lg sm:rounded-xl px-0.5 sm:px-1 text-center text-[13px] sm:text-[16px] font-bold text-[#ccff00] outline-none" />
                <input type="text" value={form.name} onChange={e => handleAutoCalc(e.target.value, form.qty)} placeholder={vocab.namePlaceholder} className="flex-1 min-w-0 bg-[#121212] border border-slate-800 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1.5 sm:py-2 text-[14px] sm:text-[16px] font-bold text-white outline-none focus:border-[#ccff00] placeholder-slate-600" />
                <div className="relative w-[65px] sm:w-24 shrink-0">
                  <input type="number" value={form.qty} onChange={e => handleAutoCalc(form.name, e.target.value)} placeholder={vocab.qtyPlaceholder} className="w-full bg-[#121212] border border-slate-800 rounded-lg sm:rounded-xl pl-1 sm:pl-2 pr-5 sm:pr-6 py-1.5 sm:py-2 text-[14px] sm:text-[16px] font-bold text-[#ccff00] outline-none focus:border-[#ccff00] placeholder-slate-600" />
              ...
              <div className="flex gap-1 sm:gap-2">
                {[{l:'Kcal',v:form.k,k:'k',c:'text-orange-500 border-orange-500/30'},{l:isFr?'Pro':'Pro',v:form.p,k:'p',c:'text-blue-500 border-blue-500/30'},{l:isFr?'Glu':'Carb',v:form.c,k:'c',c:'text-amber-500 border-amber-500/30'},{l:isFr?'Lip':'Fat',v:form.f,k:'f',c:'text-purple-500 border-purple-500/30'}].map((f, i) => (
                  <div key={i} className="flex-1 relative mt-2">
                    <span className="absolute -top-2 left-1/2 -translate-x-1/2 bg-[#0a0a0a] px-1 text-[7px] sm:text-[8px] font-bold text-slate-400 uppercase tracking-wider">{f.l}</span>
                    <input type="number" placeholder="-" value={f.v === 0 ? '' : f.v} onChange={e => setForm({...form, [f.k]: e.target.value})} className={`w-full py-1.5 sm:py-2 bg-[#121212] border rounded-lg sm:rounded-xl text-center text-[14px] sm:text-[16px] font-black outline-none ${f.c}`} />
                  </div>
                ))}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in duration-200">
          <div className="bg-[#121212] border border-slate-800 p-6 rounded-[2rem] w-full max-w-sm shadow-2xl">
            ...
            <div className="space-y-4">
              {[{label:vocab.calories,key:'calories',color:'text-orange-500'},{label:vocab.protein,key:'protein',color:'text-blue-500'},{label:vocab.carbs,key:'carbs',color:'text-amber-500'},{label:vocab.fat,key:'fat',color:'text-purple-500'}].map((item) => (
                <div key={item.key} className="flex items-center justify-between bg-[#0a0a0a] p-3 rounded-2xl border border-slate-800">
            ...
            <button onClick={() => setShowSettings(false)} className="w-full mt-6 bg-[#ccff00] text-black font-black py-4 rounded-xl hover:bg-[#b3e600] transition shadow-lg flex justify-center items-center gap-2 uppercase tracking-widest"><Target size={20} /> {vocab.saveBtn}</button>
          </div>
        </div>
      )}
    </div>
